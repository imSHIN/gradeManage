<template>
    <div>
      <section>
        ...
        <!-- <el-input placeholder="请输入查询内容" v-model="search.content" class="input-with-select" style="width:350px;margin-left:100px;display: inlin-block;" >
        ...
        <div class="border">
            <span>年级</span>
          <el-select v-model="selectGrade.value" placeholder="请选择" @change="getFilterStudents()">
            <el-option
              v-for="item in selectGrade.options"
              :key="item"
              :value="item">
            </el-option>
          </el-select>
          <span>专业</span>
          <el-select v-model="selectMajor.value" placeholder="请选择" @change="getFilterStudents()">
            <el-option
              v-for="item in selectMajor.options"
              :key="item"
              :value="item">
            </el-option>
          </el-select>
          <span>班级</span>
          <el-select v-model="selectClass.value" placeholder="班级" @change="getFilterStudents()">
            <el-option
              v-for="item in selectClass.options"
              :key="item"
              :value="item">
            </el-option>
          </el-select>
            ...
            <el-dialog title="学生成绩审核" :visible.sync="dialogVisible" width="75%" :close-on-click-modal= "false">
                ...
                <div slot="footer" class="dialog-footer">
                    ...
                    <el-button type="primary" @click="finish()">完成审核</el-button>
                </div>
            </el-dialog>
        </div>
        </section>
...
export default {
  ...
  data () {
    return {
      ...
      selectGrade: {
        // value: '2015',
        // options: [{value: '2015', label: '2015级'}, {value: '2016', label: '2016级'}, {value: '2017', label: '2017级'}]
        // options: ['2015', '2016']
      },
      selectClass: {
        // value: '软件工程1班',
        // options: [{value: '软件工程1班', label: '软件工程1班'}, {value: '土木1班', label: '土木1班'}]
      },
      selectMajor: {},
      condition: {}, // 筛选参数
      skipto: 0, // 翻页参数
  ...
  methods: {
    ...
    // },
    initStudent () {
      axios.get(this.httpPre + '/teacher/getStuInfo')
         .then(response => {
           if (response.data.status === 1 || response.data.status === 2) {
             this.$alert(response.data.statusInfo.message || '错误', {
               confirmButtonText: '确定'
           ...
           this.selectGrade = response.data.stuInfo.selectGrade
           this.selectMajor = response.data.stuInfo.selectMajor
           this.selectClass = response.data.stuInfo.selectClass
    ...
    getFilterStudents () {
      this.StudentsLoading = true
      var condition = {
        grade: this.selectGrade.value,
        major: this.selectMajor.value,
        classNo: this.selectClass.value
      }
      axios.post(this.httpPre + '/teacher/getStuUploadInfo', {
        condition
      })
          .then(response => {
            if (response.data.status === 1 || response.data.status === 2) {
