<template>
  <div class="stuinfo">
    <div class="info info-border">
      ...
      <div class="info-item">教师编号: {{userInfo.jzxId}}</div>
      ...
      <div class="info-item">职位: {{userInfo.jzxPosition}}</div>
      <div class="info-item">电话: {{userInfo.jzxTel}}</div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      dialogUpdate () {
        ...
        params.append('jzxTeacherid', localStorage.getItem('id'))
        params.append('jzxPassword', this.form.jzxPassword)
        this.$http.post('/teacheraccount/update', params)
          .then(response => {
            this.loading = false
      ...
      getUserInfo () {
        ...
        this.$http.post('/teacher/select', params)
          .then(response => {
            this.loading = false
            if (response.data === '') {
              this.$alert('发生错误', '错误', {
