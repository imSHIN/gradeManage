    ...
    methods: {
      ...
      getUserInfo () {
        this.$http.get('/user')
          .then(response => {
            this.loading = false
            if (response.data.status === 1 || response.data.status === 2) {
              this.$alert(response.data.statusInfo.message || '错误', '1', {
                confirmButtonText: '确定'
              })
              return
            } else if (response.data.status !== 0) {
      ...
      getState () {
        ...
        .then(response => {
          ...
          this.setIsValidTime(this.isValidTime)
      ...
      getScore () {
        this.$http.get('/user/getTentativeScore/' + this.user._id)
          .then(response => {
            if (response.data.status === 1 || response.data.status === 2) {
              this.$alert(response.data.statusInfo.message || '错误', '3', {
                confirmButtonText: '确定'
              })
              return
            } else if (response.data.status !== 0) {
